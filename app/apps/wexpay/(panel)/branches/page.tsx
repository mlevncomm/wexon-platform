...
export default async function WexPayBranchesPage({ searchParams }: { searchParams: SearchParams }) {
    ...
    return (
      <WexPayPanel title="Şubeler" description="Restoranlara bağlı şube kayıtları.">
        <WexPayEmptyNotice>
          ...
          <Link href="/apps/wexpay/restaurants" className="font-bold text-emerald-700">
            Restoran oluştur
          </Link>
        </WexPayEmptyNotice>
      </WexPayPanel>
  ...
  return (
    <WexPayPage>
      ...
      <div className="grid min-w-0 grid-cols-2 gap-2 sm:flex sm:flex-wrap">
        <FilterChip label="Tümü" href="/apps/wexpay/branches" active={!selectedRestaurantId} />
        {restaurants.map((restaurant) => (
          <FilterChip
            key={restaurant.id}
            label={restaurant.name}
            href={`/apps/wexpay/branches?restaurantId=${restaurant.id}`}
