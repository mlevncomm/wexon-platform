      ...
      <main className="flex min-h-screen items-center justify-center bg-[#f6f8f7] px-5 py-10">
        <section className="max-w-xl rounded-[32px] border border-slate-200 bg-white p-8 text-center shadow-xl shadow-slate-200/70">
          ...
          <p className="mt-4 text-sm font-semibold text-slate-600">
            WexHotel ve WexB2B abonelikleri sonraki fazda açılacaktır.
          </p>
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:justify-center">
            <Link href="/demo-request" className="rounded-full bg-emerald-500 px-5 py-3 text-sm font-black text-white">
              Demo talebi oluştur
            </Link>
            <Link href="/" className="rounded-full border border-slate-200 px-5 py-3 text-sm font-black text-slate-900">
              Ana sayfaya dön
            </Link>
          </div>
        </section>
      ...
      <section className="relative mx-auto grid min-h-[calc(100vh-2rem)] max-w-6xl gap-5 lg:grid-cols-[0.9fr_1fr] lg:items-center">
        <aside className="relative overflow-hidden rounded-2xl border border-white/10 bg-white/[0.04] p-5 text-white shadow-2xl shadow-slate-950/30 backdrop-blur sm:p-6 lg:p-7">
          ...
          <div className="relative">
            <Link href="/" className="mb-5 inline-flex rounded-full bg-white px-4 py-2 text-sm font-black text-slate-950">
              Wexon
            </Link>
            ...
            <p className="mt-2 text-sm font-semibold text-slate-300">
              {billingInterval === "yearly" ? "yıllık" : "aylık"} ödeme
            </p>

            <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.06] p-4">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-black text-white">WexPay {dbPlan?.name ?? planLabels[planKey]}</p>
                  <p className="mt-1 text-sm font-semibold text-slate-300">
                    {billingInterval === "yearly" ? "Yıllık olarak faturalandırılır" : "Aylık olarak faturalandırılır"}
                  </p>
                </div>
              ...
              <div className="mt-4 space-y-2 text-sm">
                <PriceRow label="Ara toplam" value={money(price.subtotal, price.currency)} />
                <PriceRow label={`KDV (%${price.taxRatePct})`} value={money(price.tax, price.currency)} />
                <div className="border-t border-white/10 pt-3">
                  <PriceRow label="Bugün ödenecek toplam" value={money(price.total, price.currency)} strong />
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 flex rounded-2xl border border-white/10 bg-white/[0.06] p-1">
              <Link
                href={`${checkoutBase}&interval=monthly`}
                className={`flex-1 rounded-xl px-4 py-2 text-center text-xs font-black ${billingInterval === "monthly" ? "bg-white text-slate-950" : "text-slate-300"}`}
              >
                Aylık
              </Link>
              <Link
                href={`${checkoutBase}&interval=yearly`}
                className={`flex-1 rounded-xl px-4 py-2 text-center text-xs font-black ${billingInterval === "yearly" ? "bg-white text-slate-950" : "text-slate-300"}`}
              >
                Yıllık
              </Link>
            </div>
            ...
            <div className="mt-5 rounded-2xl border border-emerald-400/20 bg-emerald-500/10 p-4">
              ...
              <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-300">
                Aboneliğiniz başladıktan sonra WexPay kurulum süreciniz başlatılır. Ekibimiz 5 iş günü içinde kurulum
                detaylarını netleştirmek için sizinle iletişime geçer.
              </p>
            </div>

            <div className="mt-4 grid gap-2">
              {[
                "Güvenli abonelik başlangıcı",
                accountFeature,
                "WexPay lisansı ödeme sonrası aktif edilir",
                "Kurulum süreci 5 iş günü içinde başlatılır",
              ].map((item) => (
                <div
                  key={item}
        ...
        <div className="rounded-2xl border border-white/20 bg-white p-5 shadow-2xl shadow-emerald-950/30 sm:p-6 lg:p-7">
          <h2 className="text-2xl font-black tracking-tight text-slate-950 sm:text-3xl">
            {currentUser ? "Mevcut hesabınızla aboneliği başlatın" : "WexPay aboneliğinizi başlatın"}
          </h2>
          <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-600">
            {currentUser
              ? "Bu abonelik, giriş yaptığınız Wexon Core hesabına bağlı organizasyon için başlatılacak."
              : "Ödeme sonrası Wexon Core hesabınız oluşturulur ve WexPay lisansınız aktif edilir."}
          </p>
          {currentUser && (
            <div className="mt-5 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-sm font-semibold text-emerald-900">
              {currentUser.email} hesabıyla devam ediyorsunuz. Organizasyon:{" "}
          ...
          <div className="mt-5 flex flex-wrap items-center justify-between gap-3 text-xs font-semibold text-slate-500">
            <div className="flex flex-wrap gap-3">
              ...
              {currentUser && (
                <Link href="/dashboard" className="font-bold text-slate-600 hover:text-slate-950">
                  Core paneline dön
                </Link>
              )}
            </div>
...
function PriceRow({ label, value, strong = false }: { label: string; value: string; strong?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-4">
      <span className={strong ? "font-black text-white" : "font-semibold text-slate-300"}>{label}</span>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        readOnly={readOnly}
        className={`mt-1.5 w-full rounded-2xl border border-slate-200 px-4 py-2.5 text-sm font-semibold outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100 ${readOnly ? "bg-slate-50 text-slate-500" : ""}`}
      />
    </label>
  );
}
