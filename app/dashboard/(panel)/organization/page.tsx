...
import {
  DashboardOrganizationTabBar,
  DashboardOrganizationTabPanel,
  type OrganizationTabId,
} from "@/components/marketing/DashboardOrganizationTabs";
import {
...
function readOrganizationTab(value: string | undefined): OrganizationTabId {
  if (value === "edit" || value === "businesses") return value;
  return "overview";
}
...
export default async function DashboardOrganizationPage({ searchParams }: { searchParams: DashboardSearchParams }) {
  ...
  const tab = readOrganizationTab(params.tab);
  const baseHref = dashboardHref("/dashboard/organization", organizationContext);
    ...
    <div className="space-y-6 sm:space-y-8">
      ...
      <DashboardPanel>
        <Suspense fallback={<div className="h-10" aria-hidden />}>
          <DashboardOrganizationTabBar baseHref={baseHref} />
        </Suspense>

        <DashboardOrganizationTabPanel tab="overview" current={tab}>
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <DashboardInfoRow label="Bağlı işletme" value={organization.restaurants.length} />
            <DashboardInfoRow label="Üyelik" value={organization.memberships.length} />
          </div>
        </DashboardOrganizationTabPanel>

        <DashboardOrganizationTabPanel tab="edit" current={tab}>
          <DashboardSectionTitle
            badge="Self-service"
          ...
          {canEdit ? (
            <form action={updateCustomerOrganizationAction} className="grid gap-4 md:grid-cols-2">
              ...
              <label className="block">
                ...
                <input
                  name="taxNo"
                  defaultValue={organization.taxNo ?? ""}
                  className="mt-2 w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm font-semibold outline-none focus:border-emerald-400 focus:ring-4 focus:ring-emerald-100"
                />
              </label>
              <label className="block">
        ...
        <DashboardOrganizationTabPanel tab="businesses" current={tab}>
          ...
          {organization.restaurants.length === 0 ? (
            <DashboardEmptyState
              title="Bağlı işletme yok."
              description="WexPay operasyonlarına bağlı restoranlar burada listelenir."
            />
          ) : (
              ...
              {organization.restaurants.map((restaurant) => (
                <div key={restaurant.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <DashboardStatusPill active={restaurant.isActive}>
                      {restaurant.isActive ? "Aktif" : "Pasif"}
                    </DashboardStatusPill>
                  </div>
                  <p className="mt-3 text-xs font-semibold text-slate-500">Şube sayısı: {restaurant.branches.length}</p>
                </div>
              ))}
            </div>
          )}
