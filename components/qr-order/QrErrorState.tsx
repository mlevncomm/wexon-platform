...
import { qrFrameNarrow, qrGlass } from "@/components/qr-order/qr-theme";
...
export default function QrErrorState({
  ...
  return (
    <main
      className="relative isolate flex min-h-[100dvh] w-full flex-1 items-center justify-center py-12 text-slate-950 sm:py-16"
      data-testid="qr-error-state"
    >
      <div
        aria-hidden="true"
        className="pointer-events-none fixed inset-0 -z-10 bg-[linear-gradient(165deg,#F6F8F5_0%,#ECFDF5_38%,#FAFAF7_100%)]"
      />
      <div className={`${qrFrameNarrow} w-full`}>
        <div className={`${qrGlass} mx-auto w-full max-w-lg rounded-[32px] p-8 text-center sm:p-10`}>
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.18em] text-emerald-800">WexPay</p>
