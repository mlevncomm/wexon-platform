...
import { qrGhostCta, qrIconBtn } from "@/components/qr-order/qr-theme";
...
export default function QrMenuScreen({
  ...
  return (
    <div className="mx-auto w-full max-w-lg px-4 pb-28 pt-3">
      <header className="sticky top-0 z-20 -mx-4 border-b border-emerald-100/50 bg-[#F6F8F5]/85 px-4 py-3 backdrop-blur-md">
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-1 flex flex-wrap items-center gap-2">
              <p className="truncate text-xs font-semibold text-slate-500">{context.tableLabel}</p>
      ...
      <div className="mt-4 space-y-3" data-testid="qr-menu-screen">
        {visibleProducts.length === 0 ? (
          <div className="rounded-[28px] border border-dashed border-slate-300/80 bg-white/70 p-8 text-center backdrop-blur-sm">
      ...
      <button type="button" onClick={onCallWaiter} className={`${qrGhostCta} mt-6`}>
        Garson çağır
      </button>
