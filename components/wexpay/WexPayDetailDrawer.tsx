...
export default function WexPayDetailDrawer({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[70]" role="presentation">
      <button type="button" className="absolute inset-0 bg-slate-950/40" aria-label="Paneli kapat" onClick={close} />
      <div
        ref={panelRef}
        role="dialog"
        aria-modal="true"
        aria-labelledby={titleId}
        className="absolute inset-y-0 right-0 flex h-[100dvh] max-h-[100dvh] w-full flex-col border-l border-slate-200 bg-white shadow-2xl sm:w-[min(90vw,40rem)] xl:w-[42rem]"
      >
        ...
        {footer ? (
          <footer className="sticky bottom-0 shrink-0 border-t border-slate-200 bg-white px-4 py-4 sm:px-6">{footer}</footer>
        ) : null}
      </div>
    </div>,
    document.body,
